...
void create_packet_struct(uint8_t packet_array[], struct packet *pp)
{
  ...
  if (packet_array[0] & X_SIGN)
    pp->delta_x = (packet_array[1] | 0xFF00);
  else
    pp->delta_x = packet_array[1];

  // Determines if x is in two's complement
  if (packet_array[0] & Y_SIGN)
    pp->delta_y = (packet_array[2] | 0xFF00);
  else
    pp->delta_y = packet_array[2];
}
...
int mouse_issue_command(uint8_t cmd) {
  ...
  while(acknowledgment_byte == NACK && num_tries < MAX_TRIES) {
    ...
    if(kbc_issue_command_with_arg(WRITE_MOUSE, cmd)){
      return 1;
    }
...
int mouse_set_stream_mode(){
  return mouse_issue_command(STREAM_MODE);
}


int mouse_enable_data_rep(){
  return mouse_issue_command(ENABLE_DATA_REPORT);
}


int mouse_disable_data_rep(){
  return mouse_issue_command(DISABLE_DATA_REPORT);
}
