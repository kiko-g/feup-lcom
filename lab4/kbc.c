...
int kbc_issue_command(uint8_t cmd) 
{
  ...
  {
		if(util_sys_inb(KBC_STATUS_PORT, &status)) return 1; 	
...
int kbc_read_return_value(uint8_t *value) 
{
	...
	while(num_tries < MAX_TRIES) 
  {
		...
		if(status & OUT_BUF_FULL) 
    {
			...
			if(util_sys_inb(KBC_OUT_BUF, value)) return 1;
...
int kbc_read_acknowledgment(uint8_t *acknowledgment_byte) 
{
  ...
  {
    num_tries++;
	  if(util_sys_inb(KBC_STATUS_PORT, &status)) continue;
    if((status & OUT_BUF_FULL) == 0) continue;
    if(util_sys_inb(KBC_OUT_BUF, acknowledgment_byte)) continue;

    if((*acknowledgment_byte != ACK) 
    && (*acknowledgment_byte != NACK) 
    && (*acknowledgment_byte != ERROR)) continue;
