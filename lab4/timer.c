...
{
    *bit_no = BIT(timer_hook_id); //sets the bitmask for irq_set

    if(sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &timer_hook_id) != OK) { //function returns OK on success
        return 1;
    }
...
{
    if(sys_irqrmpolicy(&timer_hook_id) != OK) { //function returns OK on success 
      return 1;
    }
    else return 0;
}
